...
import 'package:suplo_project_8_12_2020/app/blocs/product/product.responsity.dart';

class ProductBloc {
  final reponsity = ProductRepository();
  final productFetcher = PublishSubject<ProductModel>();
  Stream<ProductModel> get allProduct => productFetcher.stream;

  fetchProduct(String url) async {
    url = url?.trim();
    if (url != null) {
      ProductModel productModel = await reponsity.getProductDetail(url);
      productFetcher.sink.add(productModel);
    }
  }

  dispose() {
    productFetcher.close();
  }
}
